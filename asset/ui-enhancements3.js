...
function lazyLoadSections() {
    const observer = new IntersectionObserver((entries, obs) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                if (!el.dataset.loaded) {
                    if (el.dataset.lazyChart) {
                        const chartId = el.id;
                        console.log(`Rendering lazy chart: ${chartId}`);
                        if (typeof app.renderCharts === "function") {
                            app.renderCharts();
                        }
                    }
                    if (el.dataset.lazyTable) {
                        const tableId = el.id;
                        console.log(`Rendering lazy table: ${tableId}`);
                        if (typeof app.renderLeads === "function") {
                            app.renderLeads();
                        }
                        if (typeof app.renderProperties === "function") {
        ...
        rootMargin: "0px 0px 200px 0px" // start loading a bit before it enters
    });

    document.querySelectorAll('[data-lazy-chart], [data-lazy-table]').forEach(el => {
        observer.observe(el);
    });
}
...
// =====================
function renderMilestoneBadges(tasks) {
  console.log("🚀 renderMilestoneBadges called");
  ...
  const completed = normalizedTasks.filter(t => t.__status === "done").length;
  ...
  // Load shown badges
  let shownBadges = JSON.parse(localStorage.getItem("gamify_shown_badges") || "[]");
  console.log("📋 Already shown badges:", shownBadges);

  // Render all milestones up to completed tasks
  badgeMilestones.forEach(milestone => {
    if (completed >= milestone) {
      const badge = document.createElement("div");
      badge.className = "gamify-badge";
      ...
      badge.textContent = `🏅 ${milestone} tasks completed!`;

      // Highlight new milestone
      if (!shownBadges.includes(milestone)) {
        badge.style.boxShadow = "0 0 10px #00ff00";
        shownBadges.push(milestone);
      }

      container.appendChild(badge);
      console.log("➕ Badge rendered:", badge.textContent);
    }
  });

  // Save updated milestones
  localStorage.setItem("gamify_shown_badges", JSON.stringify(shownBadges));
}
