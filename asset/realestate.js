...
const app = {
  ...
  renderAll() {
    ...
    // ----------------------
    // 6️⃣ Task Reminders (due in 24h)
    // ----------------------
    const now = new Date();
    const next24h = new Date(now.getTime() + 24 * 60 * 60 * 1000);
    const dueSoonTasks = (this.data.tasks || []).filter(t => {
      const dueDate = new Date(t.DueDate || t.dueDate || t.Due || t.due || now);
      return dueDate >= now && dueDate <= next24h && (t.Status || t.status) === "Open";
    });
    ...
    // ----------------------
    // 7️⃣ Ensure First Lead Selected
    // ----------------------
    if (!this.selectedLeadId && (this.data.leads || []).length) {
      this.selectedLeadId = this.data.leads[0].ID || this.data.leads[0].id;
    }

    // ----------------------
    // 8️⃣ Refresh CRM Dashboard
    // ----------------------
    if (typeof crmDash_refresh === "function") {
      crmDash_refresh(this.data);
    }

// Update Task Streak & Progress
if (this.data.tasks && Array.isArray(this.data.tasks)) {
  const tasks = this.data.tasks.map(t => ({
    ...t,
    __status: (t.Status || t.status || "").toLowerCase()
  }));

  const streakEl = document.getElementById("kpi-streak");
  const completed = tasks.filter(t => t.__status === "done").length;
  const total = tasks.length;

  if (streakEl) streakEl.textContent = completed;

  // Progress bar
  const progressBar = document.querySelector("#gamify-progress .progress-bar");
  if (progressBar) {
    const percentage = total ? (completed / total) * 100 : 0;
    progressBar.style.width = `${percentage}%`;

    if (percentage === 0) progressBar.style.backgroundColor = "#cccccc";
    else if (percentage < 50) progressBar.style.backgroundColor = "#f97316"; // orange
    else if (percentage < 100) progressBar.style.backgroundColor = "#3b82f6"; // blue
  ...
  if (completedPill) completedPill.textContent = completed;
  if (totalPill) totalPill.textContent = total;

  // Milestone Badges
  const container = document.getElementById("gamify-badges-container");
  if (container) {
    const badgeMilestones = [1, 5, 10, 25, 50];
    const existing = Array.from(container.children).map(b => parseInt(b.dataset.milestone));

    badgeMilestones.forEach(ms => {
      if (completed >= ms && !existing.includes(ms)) {
        const badge = document.createElement("div");
        badge.className = `gamify-badge milestone-${ms}`;
        badge.dataset.milestone = ms;
        badge.textContent = `🏅 ${ms} tasks completed!`;
...
<div style="flex:1; min-width:200px"><label>Status</label>
  <select id="lead-status">
    <option ${l.Status === 'New' ? 'selected' : ''}>New</option>
    <option ${l.Status === 'Contacted' ? 'selected' : ''}>Contacted</option>
    <option ${l.Status === 'Site Visit' ? 'selected' : ''}>Site Visit</option>
    <option ${l.Status === 'Negotiation' ? 'selected' : ''}>Negotiation</option>
...
<div style="flex:1; min-width:200px; margin-top:10px">
  ...
  <a href="sample_leads.csv" download style="font-size:12px; display:block; margin-top:5px; color:blue; text-decoration:underline;">
    Download Sample CSV
  </a>
</div>
